...
class BigramLanguageModel(nn.Module):
    ...
    def __init__(self, block_size, vocab_size, n_emb, head_size):
        super(BigramLanguageModel, self).__init__()
        # each token directly reads off the logits for the next token from a lookup table
        self.token_embedding_table = nn.Embedding(vocab_size, n_emb)
        self.position_embedding_table = nn.Embedding(block_size, n_emb)
        self.sa_head = Head(block_size,n_emb,head_size=head_size)
...
bigram = BigramLanguageModel(block_size=block_size,vocab_size=vocab_size,n_emb=64,head_size=32).to(device)
